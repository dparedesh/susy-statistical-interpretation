...
m0_vs_m12_nofloat(const char* textfile = 0, TH2D* inputHist = 0, const char* rootfile = "m0m12_nofloat.root", TString id1="m0",TString id2="m12", int   nbinsX=21,int nbinsY=17, float minX=20,float maxX=860, float minY=92.5, float maxY=347.5)
{
   ...
   cout<<"Get the harvestedtree: "<<textfile<<endl;
   ...
   //For custom binning of CLs TH2D
   bool useRefHisto(false);
   if( useRefHisto && TString(textfile).Contains("Gtt") ){
     cout << "Using custom defined CLs histo for " << textfile << endl;
     inputHist = new TH2D("hRef","", 40, 700, 2400, 200, 10, 2000 );
   }
   ...
   cout<<"%%%%%%%%%%%%% Hist mean: "<<hist->GetMean()<<endl;

   /*   for(unsigned int i=0; i<hist->GetNbinsY();i++){
     for(unsigned int j=0; j<hist->GetNbinsX();j++){
       cout<<"---------- Entry ("<<i<<","<<j<<"): "<<hist->GetBinContent(i,j)<<endl;
     }
     }*/
   if (hist!=0) {
     hist->Write();
   ...
   cout<<"%%%%%%%%%%%%% Sigp1 entries: "<<sigp1->GetEntries()<<endl;
   cout<<"%%%%%%%%%%%%% Sigp1 mean: "<<sigp1->GetMean()<<endl;
