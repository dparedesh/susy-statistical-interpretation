...
#include "contourmacros/lep_cha_limit.C"
...
#include "../../analysis/common/ATLAS_EPS_contours_new.C"
                                      ...
                                      TString fnameMass= "contourmacros/mSugraGridtanbeta10_gluinoSquarkMasses.root")
{
   ...
   if (histe_esigxsm1s!=0) histe_esigxsm1s->SetDirectory(0);
 	...
 	histecls = ExtendHistRange( histecls, "copy_histecls", 0);
	histocls = ExtendHistRange( histocls, "copy_histocls", 0);	
	hist0 = ExtendHistRange( hist0, "copy_hist0", 0);
	hist1 = ExtendHistRange( hist1, "copy_hist1", 0);
	hist3 = ExtendHistRange( hist3, "copy_hist3", 0);
	hist5 = ExtendHistRange( hist5, "copy_hist5", 0);
	hist6 = ExtendHistRange( hist6, "copy_hist6", 0);
	histe_esigxsp1s = ExtendHistRange( histe_esigxsp1s, "copy_histe_esigxsp1s", 0);
    histe_esigxsm1s = ExtendHistRange( histe_esigxsm1s, "copy_histe_esigxsm1s", 0);
  ...
  { // combined
	...
	//TH2F* contour_fix = contour;
	//TH2F* contour_obs_fix =contour_obs;   
	//TH2F* contour_ep1s_fix = contour_ep1s;
	//TH2F* contour_em1s_fix = contour_em1s;
	TH2F* my_contours[]={histecls,histocls,contour,contour_obs,contour_ep1s,contour_em1s,contour_esigxsp1s,contour_esigxsm1s};	

	for(int cont=0;cont<8;cont++)
	{
		TH2F* contour_fix = my_contours[cont];
		for (int xbin = 1; xbin <= contour_fix->GetNbinsX(); xbin++) {
			for (int ybin = 1; ybin <= contour_fix->GetNbinsY(); ybin++) {
	  
			// island 
			float x=ax->GetBinCenter(xbin),y=ay->GetBinCenter(ybin);
				// // for SR1
				// if ( x < 2600 && y < 260. || x<900 && y<300) 
				// if ( y <= 300. ) 
				// {
					// if(contour_fix->GetBinContent(xbin,ybin) <= 0) contour_fix->SetBinContent(xbin, ybin, 1.66);
					// if(y>250)cout<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				// }
				
				// if(y>=240 && y<=260 && x>=800 && x<=1200)
				// {
					// if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
				// }
				// if (cont==4 && x >= 1150 && y >= 250 && x <= 1250 && y <= 280.  ) 
				// {
					// if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
					// cout<<"combined channel "<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				// }
				// if (cont==7 && x >= 1100 && y >= 260 && x <= 1300 && y <= 310. ) 
				// {
					// if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
					// cout<<"combined channel "<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				// }
				...
				{
					if(contour_fix->GetBinContent(xbin,ybin) == 0) contour_fix->SetBinContent(xbin, ybin, 1.66);
					cout<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				}
				if (x < 1150 && y <300)
				{
				if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
				...
				if (cont==4 && x >= 1100 && y >= 230 && x <= 1400 && y <= 280.  ) 
				{
					if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
					cout<<"combined channel "<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				}
				if (cont==7 && x >= 1100 && y >= 230 && x <= 1400 && y <= 280. ) 
				{
					if(contour_fix->GetBinContent(xbin,ybin) <= 1.65) contour_fix->SetBinContent(xbin, ybin, 1.66);
					cout<<"combined channel "<<contour_fix->GetBinContent(xbin, ybin)<<endl;
				}
		...
		for (int ybin = 1; ybin <= contour_fix->GetNbinsY(); ybin++) {
			float last_nonzero_val=0;
			for (int xbin = 1; xbin <= contour_fix->GetNbinsX(); xbin++) {
				if(contour_fix->GetBinContent(xbin,ybin)!=0)
					last_nonzero_val=contour_fix->GetBinContent(xbin,ybin);
				else contour_fix->SetBinContent(xbin,ybin,last_nonzero_val);
			}
		}
    }	  
  } 
  ...
  //TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS work in progress", 100, 550., 3000., 100, 250., 700. );
  TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS internal", 100, 200., 3500., 100, 100., 700. );
  ...
  //TLegend *leg = new TLegend(0.64,0.62,0.92,0.915);
  //TLegend *leg = new TLegend(0.56,0.62,0.83,0.915);
  TLegend *leg = new TLegend(0.64,0.62,0.92,0.915);
...
/*  TLatex * s500 = new TLatex( 150, 220, "#tilde{q} (500 GeV)" );
  ...
  if (channel==2) { // combined
    cout << "removing islands in electron channel ..." << endl;
    // contour line is drawn for values at 1.64485
    TAxis* ax = contour_obs->GetXaxis();
    TAxis* ay = contour_obs->GetYaxis();
	// tb schimbat numele ca-s clonate!	
	TH2F* my_contours[]={histecls,histocls,contour,contour_obs,contour_ep1s,contour_em1s,contour_esigxsp1s,contour_esigxsm1s};	
	TFile*test_file=TFile::Open("test_contours.root","RECREATE");
	for(int cont=0;cont<8;cont++)
	{
		TH2F* contour_fix = my_contours[cont];
		for (int xbin = 1; xbin <= contour_fix->GetNbinsX(); xbin++) {
  ...
  if (discexcl==1) {
	...
	leg->AddEntry((TObject*)0, "All limits at 95% CL_{S}", "");
  ...
  //msugra_noEWSB_curve = msugra_noEWSB("../../analysis/common/noEWSB.txt");
     //msugra_noEWSB_curve = msugra_noEWSB("../../analysis/common/msugra_status.txt");
 
  TGraph* staulsp = new TGraph;
  TGraph* negmasssq = new TGraph;
  TGraph* noRGE = new TGraph;
  TGraph* noEWSB = new TGraph;
  TGraph* tachyons = new TGraph;
  
    c->cd();
  //msugraThExcl("../../analysis/common/msugra_status.txt",staulspa,staulspb,noRGE,noEWSBa,noEWSBb,tachyons,c); // old directory
  msugraThExcl("../../analysis/common/msugra_status.txt",staulsp,negmasssq,noRGE,noEWSB,tachyons,c);
 
 //msugra_noEWSB_curve = msugra_noEWSB("../../analysis/common/msugra_state.txt", 0);
 //msugra_RGE_curve = msugra_noEWSB("../../analysis/common/msugra_state.txt", 1);
  ...
  leg->AddEntry( negmasssq, "Soft SUSY ","F" ); 
  ...
  Leg1->SetTextColor( 1 );
  Leg1->DrawLatex(0.15, 0.81, Form("L^{int} = %1.2f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  Leg1->AppendPad();
  ...
  prel->DrawLatex( 0.33,0.87,"INTERNAL");   // 0.13, 0.81,// 0.27,0.87
  prel->AppendPad();
...
TH2F*ExtendHistRange( TH2F* hist, const char* name=0, const char* title=0)
{
	if(!hist) return 0;
  int nbinsx = hist->GetNbinsX();
  int nbinsy = hist->GetNbinsY();
  ...
  double bin_width=(ymax-ymin)/nbinsy;
  
  int new_nbinsy=nbinsy;
  double new_ymin=ymin;
  
  while(new_ymin>100)
	{ new_nbinsy++; new_ymin-=bin_width;}

  TH2F* hist2 = new TH2F(name, title, nbinsx, xmin, xmax, new_nbinsy, new_ymin, ymax);
  
  int yoffset=new_nbinsy-nbinsy;
  for (Int_t ibin1=1; ibin1 <= hist->GetNbinsX(); ibin1++) {
    for (Int_t ibin2=1; ibin2 <= hist->GetNbinsY(); ibin2++)
	{
      hist2->SetBinContent( ibin1, ibin2+yoffset, hist->GetBinContent(ibin1,ibin2) );
	}
  }
