...
def checkNames(histList):
  ...
  duplicate = any(samples.count(x) > 1 for x in samples)

  if duplicate:
      print "- I'm sorry this patch need modifications... please contact your SS3L HistFitter team if you see this message.."
      exit(1)

  print samples

def getStat(filename,region, sample):
  '''
  This method returns the statistical error *always before fit* for the requested sample.
  If no sample is provided, then it will sum up all the histograms found in that file 
  matching the string h*Nom_<region>_obs
  '''

  error=Double(0.)

  fileHistos = os.getenv("HFRUNDIR")+"/data/"+filename.split("/")[-2]+".root"

  print "--> Looking for statistical error in : ", fileHistos
  print "-- If not sample is provided, I will sum up all the histograms found in that file matching the string h*Nom_<region>_obs"
  print "-- signal and discovery samples are included in the total"

  nom="Nom_"+region+"_obs_"

  pFile = TFile(fileHistos)

  histoNames = [key.GetName() for key in pFile.GetListOfKeys() if nom in key.GetName()]

  checkNames(histoNames)

  pH = pFile.Get(histoNames[-1]).Clone()
  pH.Reset()
  ...
  for histo in histoNames:
      ...
      if sample is not "":
          ...
          pHName= "h"+sample+"Nom_"+region   
          
          if pHName in histo: 
              pH.Add(pFile.Get(histo))
              break

      else: pH.Add(pFile.Get(histo))
         


  print "Total: ",pH.IntegralAndError(1,pH.GetNbinsX(),error)
...
"""
  ...
  print "Input parameters for latexfitresults():"
  ...
  statError = getStat(filename,region, sample)

  nFittedInRegion=pdfInRegion.getVal()
  regSys['sqrtnfitted'] = statError #TMath.Sqrt(nFittedInRegion)
  regSys['nfitted'] = nFittedInRegion

  print '------------- CALCULATING TOTSYSERR -------------'
  pdfFittedErrInRegion = Util.GetPropagatedErrorNoStat(pdfInRegion, result, doAsym)
  print 'pdfFittedErrInRegion: ',pdfFittedErrInRegion
  ...
  if len(namemap)>0: 
    for key in namemap.keys():
      ...
      for parname in namemap[key]:
        ...
        pass
      sysError  = Util.GetPropagatedErrorNoStat(pdfInRegion, result, doAsym)
      regSys['syserr_'+key] =  sysError
      print '+++ syserr_',key,': ',sysError
      for idx in range(fpf.getSize()):
        parname = fpf[idx].GetName()
        par = w.var(parname)
        par.setConstant()
    ...
    for idx in range(fpf.getSize()):
      parname = fpf[idx].GetName()
      print "-- Looking for parameters: ",parname
  
      if "gamma_stat" not in parname:
        par = w.var(parname)
        par.setConstant(False)
        sysError  = Util.GetPropagatedErrorNoStat(pdfInRegion, result, doAsym)
        regSys['syserr_'+parname] =  sysError
        print '+++ syserr_',parname,': ',sysError
        par.setConstant() 
      else:
        print "- gamma_stat* will not be printed in the systematic tables... Skipping..."
...
if __name__ == "__main__":
  ...
  for chan in origChanList:
    ...
    if not chosenSample:
      ...
      chanSys[chan] = regSys
      #print "chanSys[chan]: ",chanSys[chan],"\n"
      #print "////////////////////\n"
      #print "syserr_alpha_FT_JVT: ",chanSys[chan]['syserr_alpha_FT_JVT']
      chanList.append(chan)
      #print "chanList: ",chanList[-1],"\n"
    else:
      for sample in sampleList:
        ...
        print "chanSys[chan]: ",chanSys[chan+"_"+sampleName],"\n"
  ...
  ##################Preparing txt file for systematic plot#######################
  outputTxtFile = chanList[-1]+"_systematic_grouped.txt"
  systDir = "{0}/../Run/SystematicTxt/".format(os.environ['HISTFITTER'])
  if not os.path.isdir(systDir):
    print systDir,"does not exits. Creaing";  os.mkdir(systDir)


  if not chosenSample:  
    g = open(systDir+outputTxtFile,'w')
    d = chanSys[chan]
    listofkeys = sorted(d.iterkeys(), key=lambda k: d[k], reverse=True)

    for name in listofkeys:
      syst_name = name
      syst_value=str(chanSys[chan][name])
      g.write(syst_name)
      g.write("\t")
      g.write(syst_value)
      g.write("\n")
    g.close()
    print "\nwrote results in file: %s"%(outputTxtFile)
