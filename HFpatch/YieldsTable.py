...
def latexfitresults(filename,regionList,sampleList,dataname='obsData',showSum=True, doAsym=True, blinded=False, splitBins=False):
  """
  ...
  pdfFittedErrInRegionList = [ Util.GetPropagatedErrorNoStat(pdf, resultAfterFit, doAsym) for pdf in rrspdfinRegionList]
  ...
  if showSum:
    ...
    nPdfSumError = Util.GetPropagatedErrorNoStat(pdfSumInAllRegions, resultAfterFit, doAsym)
  ...
  for isam, sample in enumerate(sampleList):
    ...
    for ireg, region in enumerate(regionList):
      ...
      if not sampleInRegion==None:
        ...
        sampleInRegionError = Util.GetPropagatedErrorNoStat(sampleInRegion, resultAfterFit, doAsym) 
    ...
    if showSum:
      ...
      nSampleSumError = Util.GetPropagatedErrorNoStat(sampleSumInAllRegions, resultAfterFit, doAsym)
  ...
  pdfExpErrInRegionList = [ Util.GetPropagatedErrorNoStat(pdf, resultBeforeFit, doAsym)  for pdf in rrspdfinRegionList]
  ...
  if showSum:
    ...
    nPdfSumVal = pdfSumInAllRegions.getVal()
    nPdfSumError = Util.GetPropagatedErrorNoStat(pdfSumInAllRegions, resultBeforeFit, doAsym)
  ...
  for isam, sample in enumerate(sampleList):
    ...
    for ireg, region in enumerate(regionList):
      ...
      if not MCSampleInRegion==None:
        ...
        MCSampleInRegionError = Util.GetPropagatedErrorNoStat(MCSampleInRegion, resultBeforeFit, doAsym) 
    ...
    if showSum:
      ...
      nMCSampleSumError = Util.GetPropagatedErrorNoStat(MCSampleSumInAllRegions, resultBeforeFit, doAsym)
...
if __name__ == "__main__":
  ...
  f.write( tablestart() )
  f.write( tablefragment(m3, tableName, regionsList_2Digits,sampleList_decoded,showBeforeFitError, wsFileName) )
  if tableCaption != "" or tableLabel != "":
      f.write( tableEndWithCaptionAndLabel(tableCaption, tableLabel) )
  else:
      f.write( tableend(userString,tableName,outputFileName) )
  f.close()
