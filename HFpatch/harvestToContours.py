...
def processInputFile(inputFile, outputFile, label = ""):
    ...
    if label=="_UL":
        for whichEntry in ["upperLimit","expectedUpperLimit","excludedXsec"]:
    ...
    if "none" not in args.forbiddenFunction.lower():
        if "SusyBtt" in args.inputFile:
	    resultsDict = addValuesToDict(resultsDict, "x-70-172.5", numberOfPoints=100 ,value = "mirror")# default = "mirror"  give forbidden function always x
        else:
	    resultsDict = addValuesToDict(resultsDict, args.forbiddenFunction, numberOfPoints=100 ,value = "mirror" )
	#   resultsDict = addValuesToDict(resultsDict, "x", numberOfPoints=100 ,value = "mirror")# default = "mirror"  give forbidden function always x
    ...
    if label=="_UL":
        ...
        upperLimitSurface = interpolateSurface( resultsDict, args.interpolation, args.useROOT, outputSurfaceTGraph=True, tmpListOfContours=["upperLimit"],forbiddenfunction = args.forbiddenFunction )
    ...
    outputGraphs = interpolateSurface( resultsDict , args.interpolation , args.useROOT , outputSurface=True if label=="" else False,tmpListOfContours=listOfContours,forbiddenfunction = args.forbiddenFunction)
...
def harvestToDict( harvestInputFileName = "" , tmpListOfContours = listOfContours ):
    ...
    if ".json" in harvestInputFileName:
		...
		if "excludedXsec" in sample:
		    if sample["excludedXsec"]>=0: modelDict[sampleParams]["excludedXsec"] = sample["excludedXsec"]
...
def addValuesToDict(inputDict, function, numberOfPoints = 100, value = 0):
    ...
    forbiddenFunction = ROOT.TF1("forbiddenFunction${}".format(value),function,lowerLimit,upperLimit)
...
def interpolateSurface(modelDict = {}, interpolationFunction = "linear", useROOT=False, outputSurface=False, outputSurfaceTGraph=False, tmpListOfContours=listOfContours,forbiddenfunction = "x"):
    ...
    if useROOT:
        ...
        for whichContour in tmpListOfContours:
            ...
            contourList = getContourPoints(xymeshgrid[0],xymeshgrid[1],ZI, args.level,forbiddenfunction)
...
def createTGraphFromDict(modelDict,myName,listOfFIDs=None):
    ...
    for imodel,model in enumerate(modelPoints):
        if listOfFIDs!=None:
            ...
            value = modelDict[model][myName] if (args.noSig or myName in ["upperLimit","expectedUpperLimit","excludedXsec"]) else ROOT.RooStats.SignificanceToPValue(modelDict[model][myName])
...
def getContourPoints(xi,yi,zi,level,forbiddenfunction ):

    c = plt.contour(xi,yi,zi, [level])
    contour = c.collections[0]

    contourList = []
    forbiddenFunction = ROOT.TF1("forbiddenFunction",forbiddenfunction,0,9999999)
    ...
    for i in xrange( len(contour.get_paths() ) ):
        ...
        for i_now in xrange( len(x)):
	    if y[i_now]>forbiddenFunction.Eval(x[i_now]):
	        y[i_now] = forbiddenFunction.Eval(x[i_now])
