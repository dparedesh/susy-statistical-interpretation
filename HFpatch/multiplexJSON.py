...
def main():
	...
	for filename in args.inputFiles:
		...
		if  not args.ignoreUL:

			try:
                                with open(filename.replace("Nominal","UpperLimit")) as data_file:
					print ">>> >>> Adding input file for upper limits:",filename.replace("Nominal","UpperLimit")
					data = json.load(data_file)
					df = pd.DataFrame(data, columns=data[0].keys())
					df['fID'] = filename
					databaseListUpperLimit.append(df)
			except:
				print ">>> Can't find %s"%filename.replace("Nominal","UpperLimit")
	...
	if len(databaseListUpperLimit):
		...
		writeDBOutToFile(outputDBUpperLimit,args.outputFile.replace("Nominal","UpperLimit"))
