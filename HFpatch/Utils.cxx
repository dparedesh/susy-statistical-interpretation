...
double Util::GetPropagatedErrorNoStat(RooAbsReal* var, const RooFitResult& fr, const bool& doAsym)
{
    Logger << kDEBUG << " GPP for variable = " << var->GetName() << GEndl;
    ...
    vector<int> fpf_idx ;
    for (Int_t i=0 ; i<fpf.getSize() ; i++) {
      std::string parName = fpf[i].GetName();
      if(parName.find("gamma_stat") != std::string::npos){ std::cout<<"STATISTIC ERROR! Skipping.." << std::endl; continue; }
      RooAbsArg* par = errorParams->find(fpf[i].GetName()) ;
      if (par ) {
    if (! par->isConstant() ) {
      paramList.add(*par) ;
